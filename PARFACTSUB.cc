...
FILE *f1, *F2;
mpz_t C[1540], unu, trei, d, dd1, p, J[1540], z[160], sumMpz[160], sumIntMpz[160], SUMDOI[280];
...
W = N210;
H = H1;
GEN(U, W, b, C, D, E, "/CFV210.TXT");//CFV1910.TXT, P2.TXT, PRIME2.TXT.
K0 = K;
H2 = H;
D2 = D;
...
for(f = 0; f < U; f++)
	b1[f] = b[f];
INV(b1, C, H, D, E);
for(int i = 0; i < 280; i++) mpz_inits(SUMDOI[i], NULL);
		...
		if(pDoi) {
			...
			if( (V == 89) || (V == 107) || (V == 127) )				
				for(h = E; h; ) {
					...
					if(V == pDoi) {
						mpz_mul(prodnum[f], prodnum[f], C[h]);
						mpz_mul(ab[f], ab[f], C[h]);
						mpz_mul_2exp(sm[f],  sm[f], pDoi);
						}	
					else if(mpz_divisible_p(p, C[h])) {
						mpz_mul(catnum[f], catnum[f], C[h]);
						mpz_mul_2exp(ab[f], ab[f], pDoi);
						mpz_mul(sm[f],  sm[f], C[h]);
						}
				
					mpz_gcd(e1[f], sm[f], ab[f]);
					if(mpz_cmp_ui(e1[f], 1)) {
						mpz_divexact(sm[f], sm[f], e1[f]);
						mpz_divexact(ab[f], ab[f], e1[f]);
					}
				}
			}
