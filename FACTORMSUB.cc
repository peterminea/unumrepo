...
mpz_t unu, doi, trei, d, d1, p, prodnum, catnum, divdiv[U][3], divsum[U][3];
...
int jos = strtol(arg[1], &tt, 10), sus = strtol(arg[2], &tt, 10), A2[160], P1[U], putImpl[480], P[U], f, h, Y, W, D, e, H, l, I, r, pDoi, doiSus, s, S, ssq, u, v, V, H2, D2, gm;
long PP, R;
FILE *f1, *F2, *Q2, *Q3;
	...
	while(1) {
		...
		if(pDoi) {
			...
			if( (!doiSus) && (pDoi > S) || (doiSus && (63 < S + pDoi)) )
				goto s2;
			doiSus? PDOIMSUS(prodnum, trei,  pDoi, SUMDOI1, S) : PDOIMJOS(catnum, trei, pDoi, SUMDOI1, S);
			}
		h = 0;
		while(1) {//Înmulțire
			if(!r--)
