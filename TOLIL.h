...
int PREPFIS(const char *T, int b1);
void GENGEN(gmp_ui *b, mpz_t *C, int &D, int &E, int &H, int &K, int &S, int b1, int inv);
...
void RAT(FILE *f1, int a1, int b1, char *Y, char &w1, unsigned long P, unsigned long &PP, unsigned long &PP1, int mod, char p2, int num);
...
//FUNCȚII LA FILTRARE:
void SMVPAR2(mpz_t D, mpz_t d1, mpz_t e1, mpz_t p1, FILE *S2, gmp_ui *b, gmp_ui *b2, int i, int j, gmp_ui div1[][MAX2], gmp_ui sum1[][MAX2], gmp_ui *sumdoi, char *Y, int X);
void PURG1PAR2(mpz_t D, mpz_t d1, mpz_t e1, mpz_t p1, FILE *S2, gmp_ui *b, gmp_ui *b2, int i, int j, gmp_ui div1[][MAX2], gmp_ui sum1[][MAX2], mpz_t *q1, char *Y, int X);
void SUMVPAR2(mpz_t D, mpz_t d1, mpz_t e1, mpz_t p1, FILE *S2, gmp_ui *b, gmp_ui *b2, int i, int j, gmp_ui div1[][MAX2], gmp_ui sum1[][MAX2], mpz_t *q1, char *Y, int X);
void DIVV1PAR2(mpz_t D, mpz_t d1, mpz_t e1, mpz_t p1, FILE *S2, gmp_ui *b, gmp_ui *b2, int i, int j, gmp_ui div1[][MAX2], gmp_ui sum1[][MAX2], mpz_t *q1, mpz_t *C, mpz_t *C1, mpz_t div[][MAX1], mpz_t sum[][MAX1], int *mem, int J, int  l, char *Y, int X);
void DIVVPAR2(mpz_t D, mpz_t d1, mpz_t e1, mpz_t p1, FILE *S2, gmp_ui *b, gmp_ui *b2, int i, int j, gmp_ui div1[][MAX2], gmp_ui sum1[][MAX2], mpz_t *q1, mpz_t *C, mpz_t *C1, mpz_t div[][MAX1], mpz_t sum[][MAX1], int *mem, int J, int l, char *Y, int X);
void IMENSPAR2(mpz_t D, mpz_t d1, mpz_t e1, mpz_t p1, FILE *S2, gmp_ui *b, gmp_ui *b2, int i, int j, gmp_ui div1[][MAX2], gmp_ui sum1[][MAX2], mpz_t *q1, mpz_t *C, mpz_t *C1, mpz_t div[][MAX1], mpz_t sum[][MAX1], int *mem, int J, int l, char *Y, int X);
...
if(J)	{
	...
	: (
		b1 <= 52?
		sprintf(CP, "cp CFV%d.TXT /CF%d.TXT && cp VPV%d.TXT /VP%d.TXT && cp PUTERIV%d.TXT /PUTERIV%d.TXT", b1, b1, b1, b1, b1, b1)
			: sprintf(CP, "cp CFV%d.TXT /CFV%d.TXT && cp VCV%d.TXT /VCV%d.TXT && cp VPV%d.TXT /VPV%d.TXT && cp PUTERIV%d.TXT /PUTERIV%d.TXT", b1, b1, b1, b1, b1, b1, b1, b1));
	system(CP);
	system("g++ -w -m64 CARONTE.cc -lgmp -fopenmp -o CARONTE");
	}
...
void PUTMAX(const char *v, gmp_ui *MX, mpz_t *MXM, int *EXP, int K, int H) {printf("TODXILIS %s\n", v);
FILE *F = fopen(v, "r");
char t;
int i = 0, j;
...
b1 < 52? sprintf(v, "/PUTERIM.TXT") : sprintf(v, "/PUTERIV%d.TXT", b1);
int K = PREPFIS("/K.TXT", b1);
...
b1 < 52? sprintf(v, "VPP.TXT") : sprintf(v, "VPV%d.TXT", b1);
...
QS2(c, 0, f);
l = 0;
b1 < 52? sprintf(v, "VCP.TXT") : sprintf(v, "VCV%d.TXT", b1);
F1 = fopen(v, "w");
i = f - 1;
...
fclose(F);
inv == 1? INV(b, C, H, D, E) : inv == 0? INV2(0, 0, b, H, D) : GOL();
if(b1 < 161) K = S = 0;
//exit(0);
}
...
//Așa că la numerele care deja au măcar 2^64: mepezetea pentru 2. Și dacă se separă de nm-ul și nr-ul celorlalți factori primi, acolo la ei nu se va întâmpla iar să se ajungă la numitor + numărător mai mari de 64 de biți? Și dacă da, atunci când? Și trebuie înmulțire mpz_t cu mepezeteaua de la 2.
...
if(a < b) {c = b%a; b = a;}//40, 225, c e 25, b 40.
while(1) {// a 40, b 25, c 15; a 25, b 15, c 10; a 15, b 10, c 5; a 10, b 5, c 0 şi cu asta basta - return b = 5.
//Cinci paşi. Dar cu algoritmul de scăderi repetate? Nu se mai împarte cu mod, doar aritmetice simple de scădere, dar dacă sunt prea mulţi paşi? Depinde de cât e de mare diferenţa între numere.
//40 cu 225 sunt cât de cât, dar dacă în locul lui 225 e ceva milionic? Scazi de te saturi.
//Şi la 40 cu 225 sunt 8 paşi.
  if(!c)
	return b;
    a = b;
    b = c;
...
//Acolo trebuiesc schimbate TRAT, VALID4 și funcțiile de la MODPRIM(E) din Z.h (adică MODPINTSUS, JOS, MPZSUS, JOS). Va fi nevoie de parametru nou mpz_t.
...
int PURO(gmp_ui A2, gmp_ui *b, int D) {
while(!(A2 & 1)) A2 >>= 1;

for(int i = 0; (i < D) && (A2 ^ 1); i++) {
	while(!(A2 % b[i])) A2 /= b[i];
	}
...
//for(int i = 0; i < D; i++) printf("%llu ", b1[i]);
//putchar('\n');
//La complementar (MODIFSUM) disc e 0.
int k = 0, l;
gmp_ui n, bb[DTOT];
//ADPRIM(bb);
...
//!disc e MODIFSUM, A2 sunt compuşii, A3 primele.
for(e = 0; e < f; e++) {
	for(j = 0; j < u1; j++)
...
//THREADRIPPER la AORUS, AORUS la XPOWER, XPOWER la THREADRIPPER, THREADRIPPER la AORUS.

gmp_ui LITUAN2(char A[][DIMM], FILE *FLIT, long D, char *tx, int min, int max) {//return;//De separat; ideal - de separat și LITUAN-ul de la VECUNG de actualizarea fișierelor.
	char Y[3000];
	D = LUNGHEZ2(tx);
	if(D < 100) return 0;
	long V = 0;
...
void RAT(FILE *f1, int b1, char *Y, char &w1, unsigned long P, unsigned long &PP, unsigned long &PP1, int mod, char p2, int num) {
	PP = ftell(f1);
	PP1 = URC1(f1, b1, Y, w1, P < 1000000000U? P/10 : 500000000L, P, (mod % 2? ':' : p2) );
	//printf("PP = %lu , PP1 = %lu .\n", PP, PP1);
...
void RAT2(FILE *f1, int b1, char *Y, char &w1, unsigned long P, unsigned long &PP, unsigned long &PP1, int mod, char p1, char p2, char scif2, int num) {//p1 3-scif1 9, p2 5-scif2 0. Dar de ce?	
	...
	do {
		PP1 = URC2(f1, b1, Y, w1, P < 1000000000U? P/10 : 500000000L, P, (scif2 == ':'? p2 : p1), scif2); 
...
gmp_ui PREGTEXT(FILE *FLIT, gmp_ui n, int dim, gmp_ui D, int min, int max, char *Y, char *tx) {
	...
	fclose(FLIT);
	system("rm LITUANULE");
	system("g++ -w -mavx512f /LITUAN2.cc -lgmp -fopenmp -o /LITUANULE");
	sprintf(Y, "/LITUANULE %s b", tx);
	system(Y);
	num = LUNGHEZ2(tx);

	if(!num || (num == D) ) {//LITUAN2 a eșuat, dimensiunea nu s-a schimbat în sortare ori e covrig.
		return 0;
	}
...
void DECIDTEXT2(char *PUT, int sus) {
	...
	if(sus <= 170) {		
		sprintf(PUT, "/PUTERIV170.TXT");
	} else
	if(sus <= 180) {		
		sprintf(PUT, "/PUTERIV180.TXT");
	} else
	if(sus <= 190) {		
		sprintf(PUT, "/PUTERIV190.TXT");
	} else
	if(sus <= 200) {		
		sprintf(PUT, "/PUTERIV200.TXT");
	} else
	if(sus <= 210) {		
		sprintf(PUT, "/PUTERIV210.TXT");
	} else
	if(sus <= 220) {		
		sprintf(PUT, "/PUTERIV220.TXT");
	} else
	if(sus <= 240) {
		sprintf(PUT, "/PUTERIV240.TXT");
	} else
	if(sus <= 270) {
		sprintf(PUT, "/PUTERIV270.TXT");
	} else
	if(sus <= 300) {
		sprintf(PUT, "/PUTERIV300.TXT");
	} else
	if(sus <= 500) {
		sprintf(PUT, "/PUTERIV500.TXT");
	} else
	if(sus <= 1000) {
		sprintf(PUT, "/PUTERIV1000.TXT");
	} else
	if(sus <= 1910) {
...
void UNIFACTSUBM(int sus, gmp_ui *b, gmp_ui *b1, gmp_ui *b2, mpz_t *C, mpz_t *J, int D, int E, int H, int K, mpz_t div[][MAX1], mpz_t sum[][MAX1], mpz_t divdiv[][3], mpz_t divsum[][3], gmp_ui div1[][MAX2], gmp_ui sum1[][MAX2], int *mem, int *P, int *P1, mpz_t unu, mpz_t d, mpz_t trei, mpz_t *SUMDOI, int sumcap, int inv, int lim) {
	mpz_t xe, g;
	mpz_inits(xe, g, NULL);
	int log, f, sb, d1, i = 0, j, k, jj, jk, dec, ll, ind[2580], PP;
	char t, PUT[90];
	FILE *F1, *F2;
	DECIDTEXT2(PUT, sus);
	//puts(PUT);
...
//FALX
	 ...
	 for(i = 0; i < H; i++) {//H descotfactizat.
		...
		for(jj = 1, jk = 0, j = 1; jj < MAX1; j++) {
			...
			if(j < 10+ind[i]) {
				if(j == 1) {
					div1[k][0] = sum1[k][0] = 1; 
					if(d1 < K) { mpz_set_ui(div[dec][0], b1[i]); mpz_set_ui(sum[dec][0], b1[i] + 1); }//Sau 1.
					}
	...
	for(i = H-K; i < H; i++) {
	for(j = 0; j < MAX1; j++) {
	if(!mpz_cmp_ui(div[i-dec][j], lim)) {P1[i] = (--j + mem[i]); break;}}}
...
void SMVPAR2(mpz_t D, mpz_t d1, mpz_t e1, mpz_t p1, FILE *S2, gmp_ui *b, gmp_ui *b2, int i, int j, gmp_ui div1[][MAX2], gmp_ui sum1[][MAX2], gmp_ui *sumdoi, char *Y, int X) {
	...
	if( (mpz_cmp(e1, D) == 1) && (VALID4(D, d1, e1) == 1) )
		#pragma omp critical
		{SCRIE(D, Y, S2);}
}
...
void TRUNCTEXT(gmp_ui P1, gmp_ui PP1, char *SCR, FILE *f1, int mod, int a1, char w1, char cifr, char cifr2) {
char Y[2000];
	if(P1 = LUNGHEZ2(SCR)) {
		f1 = fopen(SCR, "r");
