...
void VALD(mpz_t D, mpz_t d1, mpz_t e1, mpz_t E, gmp_ui nr, gmp_ui nm, char *Y, FILE *S2) {
mpz_gcd(d1, D, e1);
mpz_divexact(E, e1, d1);
mpz_divexact(d1, D, d1);
if(!mpz_cmp_ui(E, nr = mpz_get_ui(E)) && PRAG > (nm = mpz_get_ui(d1))) {
    gmp_sprintf(Y, "%Zd\n%llu\n%llu\n", D, nr, nm);
    fputs(Y, S2);
    }
}
...
INIT(K2, div, sum);
GENGEN(b, C, i, J, j, l, S, disc, disc > 210);
...
switch(disc) {
	...
	if( (strlen(Y) >= a1) && (strlen(Y) < b1) ) {
		...
		if( (mpz_cmp(e1, D) == 1) ) VALD(D, d1, e1, E, 0, 0, Y, S2);
		}
