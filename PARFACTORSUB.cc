...
FILE *f1, *F2;
mpz_t C[1540], unu, trei, d, p, J[1540], z[160], verifMpz[160], sumMpz[160], sumIntMpz[160], SUMDOI[280];
		...
		if(pDoi) {
			...
			if( (V == 89) || (V == 107) || (V == 127) )			
				...
				if(V == pDoi) {
					...
					mpz_mul_2exp(treii[f], treii[f], pDoi);
					}	
				else if(mpz_divisible_p(p, C[h])) {
					mpz_mul(catnum[f], catnum[f], C[h]);
					mpz_tdiv_q_2exp(treii[f], treii[f], pDoi);
					}
				}	
			}
